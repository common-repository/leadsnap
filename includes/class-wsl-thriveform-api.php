...
class Wncrm_thriveform_Api extends Wncrm_Base{
    ...
    public function get_source_data(){
        $company_id = wncrm_get_setting('send_to_company');
        return array(
                   'id' => $company_id,
                   'website' =>  site_url(),
    ...
    public function set_submission_instance($submission){
        ...
        foreach($submission as $key=>$value){
            if($key != "_submit_option" && $key != "_sendParams" && 
            $key != "__tcb_lg_msg" && $key != "_api_custom_fields" && 
            $key != "tve_mapping" && $key != "tve_labels" && 
            $key != "_back_url" && $key != "action" && $key != "url" && 
            $key != "_tcb_id" && $key != "tcb_token" && $key != "post_id"){
        ...
        $form_settings["id"] = $submission["post_id"];
        $form_settings["field_id"] = $submission['_tcb_id'];
    ...
    public function send_call(){
        ...
        $wncrm_api = new Wncrm_Api();
        $response = $wncrm_api->call($url,$this->mapped_data);
        return $response;
    }
    ...
    public function process_result($result){
        $data = $result;
        $send_to_company = wncrm_get_setting('send_to_company');
        if($send_to_company && $send_to_company != 0){
            //All good
        }else{
            if(isset($data->source_id)){
                wncrm_update_setting('send_to_company',$data->source_id);
            }
        }
        if(isset($data->source_id)){
            update_option('wncrm_source_id', $data->source_id, false);
        }
    }

    public function get_field_data($fields_data){
        $fields = array();
        $count = 0;
