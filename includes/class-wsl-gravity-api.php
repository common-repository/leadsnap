...
class Wncrm_Gravity_Api extends Wncrm_Base{
    ...
    public function __construct($entry, $form) {
        ...
        foreach($form['fields'] as $key=>$field){
            ...
            $field_val = $entry[$field->id];
            if(is_array($field['inputs'])){
                $vals = array();
                foreach($field->inputs as $inputs){
                    if($field->type == 'address' && $entry[$inputs['id']]){
                        $field_val[$inputs['label']] = $entry[$inputs['id']];
                    }elseif($entry[$inputs['id']]){
                        $vals[] = $entry[$inputs['id']];
                    }
                }
                //$saperator = $field->type == 'address' ? ", " : " ";
                if($field->type != 'address'){
                    $saperator = ",";
                    $field_val = implode($saperator,$vals);
                }
            }
    ...
    public function set_meta(){
        ...
        global $wp;
        $this->meta = array(
            'form_title' => $this->submission['title'],
