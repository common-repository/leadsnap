...
class Wncrm_CalderaForms_Api extends Wncrm_Base{
    ...
    public function set_submission_instance($submission, $formData){
        ...
        foreach($submission as $key=>$value){
            if($value["type"] != "button"){
                if($value["type"] == "text" || $value["type"] == "paragraph"){
                    $type = strtolower($value["label"]);
                }else{
                    $type = $value["type"];
                }
                $fields_for_sub[] = array(
        ...
        $form_settings["id"] = $formData["formId"];
        $form_settings["field_id"] = $formData['_cf_frm_id'];
    ...
    public function get_field_data($fields_data, $formData){
        $fields = array();
