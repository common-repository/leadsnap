...
class Wncrm_Typeform_Api extends Wncrm_Base{
    ...
    public function set_submission_instance($submission){
        ...
        foreach($submission[0]["answers"] as $k=>$v){
            if($v["type"] != "email"){
                $type = $v["field"]["ref"];
            }else{
                $type = $v["type"];
        ...
        $form_settings["id"] = $submission[0]["landing_id"];
        $form_settings["field_id"] = $submission[0]['response_id'];
    ...
    public function get_field_data($formData){
        ...
        foreach($formData[0]["answers"] as $k=>$v){
            if($v["type"] != "email"){

                $tags = explode('-' , $v["field"]["ref"]);
